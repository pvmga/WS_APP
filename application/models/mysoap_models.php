...
class Mysoap_models extends CI_Model {
    ...
    public function getClientesModels($vendedor, $pesquisa = NULL) {
        ...
        $sql = "SELECT 
				...
				WHERE
					C.ECOMMERCE = ? AND C.VENDEDOR_EXTERNO = ?
					$where";

        $query = $this->db->query($sql, array('S', $vendedor));
